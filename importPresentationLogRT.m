function [out2, out1] = importPresentationLogRT(fileName, extraNames)
% Changed this log file to skip the default logs and go through the
% reaction time logs.
% P.G. Mihai, 2016
%
...
convNames   = {'Trial', 'Time', 'TTime', 'Uncertainty', 'Duration', ...
    'ReqTime', 'ReqDur', 'RT', 'RT Uncertainty'};   % Defines entries that are numeric and not a string
convNames2   = {'Trial', 'Time_4', 'TTime', 'Uncertainty_5', 'Duration', ...
    'ReqTime', 'ReqDur', 'RT', 'RT Uncertainty'};   % Defines entries that are numeric and not a string
...
% Get rid of another white line.
header{4} = fgetl(fid);

%% Get variable names.
[numEntries, indexEntries, logLine] = sepHeader(fid);
names2 = {}; % Need to specify a new names variable, in order to allow
             % comparison with the first names variable, without
             % overwriting the original.
             
% Only go through these steps if there is more information at the bottom of
% the first log-file table.
if ~isempty(numEntries)
    ...
    for i = 1:numEntries
        ...
        % Finally copy entry to names2.
        names2{i} = tmp;   % remove tab
    end
    ...
    try
        while(ischar(logLine) && ~isempty(logLine))
            ...
            for i=1:numEntries
                ...
                % as double. Otherwise as string
                k=1;
                while(k<=length(convNames) && ~strcmpi(convNames2{k},names2{i}))
                    k=k+1;
                end
                
                %% Copy entries to struct
                if(k<=length(convNames))
                    out1.(names2{i})(j,:)    = str2double(tmp);
                    out2(j).(names2{i})      = out1.(names2{i})(j,:);
                else
                    % Copy to output struct
                    out1.(names2{i}){j,:}    = tmp;
                    out2(j).(names2{i})      = tmp;
